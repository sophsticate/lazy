...
class HMM(object):
    ...
    def train(self,path):
        ...
        with open(path,encoding='utf8') as f:
            for line in f:
                ...
                for k,v in enumerate(line_state):
                    Count_dict[v] +=1
                    if k ==0:
                        self.pi_dict[v] +=1 #每句话的第一个子的状态，用于计算初始状态
                    else:
                        self.A_dict[line_state[k-1]][v] +=1 #计算转移矩阵
                    self.B_dict[line_state[k]][word_list[k]] = self.B_dict[
                                                line_state[k]].get(word_list[k],0) +1.0
        ...
        #加1平滑
        self.B_dict = {k:{k1:(v1+1.0)/Count_dict[k] for k1,v1 in v.items()}
...
text = '这是一个非常棒的方案！'
text = "首先我要吐槽一下，看程序的过程中遇见了yield这个关键字，然后百度的时候，发现没有一个能简单的让我懂的，讲起来真TM的都是头头是道，什么参数，什么传递的，还口口声声说自己的教程是最简单的，最浅显易懂的，我就想问没有有考虑过读者的感受"
# prob,pos_list = hmm.viterbi(text,hmm.state_list,hmm.pi_dict,hmm.A_dict,hmm.B_dict)
# print(prob)
# print(pos_list)
res = hmm.cut(text)
print(text)
